...
"""
The `run` function configures and executes a Federated Learning (FL) experiment based on user-defined strategies and parameters.
It begins by setting a global random seed for reproducibility and loading configurations from a YAML file, which defines server 
and client settings. These configurations include FL-specific parameters, such as the number of clients, rounds, and participation ratio.

The function supports multiple FL strategies, including `FedAvg`, `FedROD`, `FedNH`, `FedProto`, `FedRep`, `FedBABU`, `Ditto`, 
`FedPer`, and `CReFF`, and initializes the appropriate client and server classes accordingly. It further allows tuning hyperparameters 
based on the chosen strategy. Additionally, a unique identifier (`run_tag`) is generated for each run based on configuration settings, 
and a results directory is created to store outputs.

If `wandb` logging is enabled, the function initializes a `wandb` session to log experiment metrics. Then, it initializes clients 
and assigns datasets based on partition schemes (e.g., `noniid-label-distribution` or `noniid-label-quantity`). For non-local 
strategies, the server object orchestrates the training rounds and saves the best global model and final server state. In the case 
of local strategy, each client trains independently without a central server, with results stored separately.

Command-line arguments allow configuration of various hyperparameters, optimizer settings, model architecture, and dataset 
partitioning schemes, offering flexibility to test multiple FL configurations. 
"""
