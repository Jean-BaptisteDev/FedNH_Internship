...
#         self.split_dir = os.path.join(self.root, self.split)
#         if not os.path.exists(self.split_dir) or len(os.listdir(self.split_dir)) == 0:
#             raise FileNotFoundError(f"Directory {self.split_dir} does not exist or is empty.")
...
#                 label = class_to_idx[class_name.lower()]  # Map class name to numeric label
...
#         self.targets = self.labels  # Ensuring `targets` attribute is available
...
def get_datasets(datasetname, **kwargs):
    """
    Loads and preprocesses various datasets based on the specified dataset name. Supports dataset-specific
    transformations and normalizations, and returns the training and test sets for each dataset.
    
    Parameters:
    - datasetname (str): The name of the dataset to load (e.g., 'FashionMnist', 'Cifar10', etc.).
    - kwargs: Additional keyword arguments for dataset-specific options (e.g., 'generator_path' for GAN-based datasets).
    
    Returns:
    - trainset (Dataset): The training dataset, preprocessed according to dataset specifications.
    - testset (Dataset): The test dataset, preprocessed according to dataset specifications.
    - invTrans (transforms.Compose or None): Optional inverse transformation for denormalizing images (available for CIFAR-10).
    ...
    - Cifar100
    - TinyImageNet
    - Cifar10Aug (augmented version of CIFAR-10)
    - GanEnhancedCifar10 (CIFAR-10 enhanced with a GAN-generated dataset)
    """
    
    invTrans = None  # Initialize inverse transformation (used for denormalization)

    if datasetname == "FashionMnist":
        # Transformation for FashionMNIST: grayscale normalization with mean and std for this dataset
        ...
        # Transformations for CIFAR-10: includes data augmentation for training
        ...
        # Inverse transformation to revert normalization for visualizations
        ...
        # Transformations for CIFAR-100: similar to CIFAR-10 but with distinct normalization values
        transform_train = transforms.Compose([
        ...
        # Load CIFAR-100 with transformations
        ...
        # Transformations for Tiny ImageNet: similar preprocessing with larger crop size
        transform_train = transforms.Compose([
        ...
        # Augmented CIFAR-10 transformations: Based on a federated learning study
        transform_train = transforms.Compose([
            transforms.RandomCrop(32, padding=4),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
        ...
        # Load CIFAR-10 augmented version
        trainset = torchvision.datasets.CIFAR10(root='~/data', train=True,
                                                download=True, transform=transform_train)
        testset = torchvision.datasets.CIFAR10(root='~/data', train=False,
                                               download=True, transform=transform_test)
        ...
        # GAN-enhanced CIFAR-10 transformations: Load pre-processed GAN-generated dataset with CIFAR-10 test set
        ...
        # Transformation for CIFAR-10 test set
        transform = transforms.Compose([
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
        ])
        
        # Load test set for CIFAR-10
        testset = torchvision.datasets.CIFAR10(root='~/data', train=False,
                                               download=True, transform=transform)
        testset.targets = torch.tensor(testset.targets)
    ...
    # if datasetname == "BrainTumorMRI":
    #     transform_train = transforms.Compose([
    #         transforms.Resize((256, 256)),
    #         transforms.Grayscale(num_output_channels=1),  # Assurez-vous que les images sont en niveaux de gris avec 1 canal
    #         transforms.ToTensor(),
    #         transforms.Normalize(mean=[0.5], std=[0.5]),  # Valeurs adaptées aux images en niveaux de gris
    #     ])
    #     transform_test = transforms.Compose([
    #         transforms.Resize((256, 256)),
    ...
    #     ])
    #     trainset = BrainTumorMRIDataset(root='/home/jbb/data/brain_tumor_mri', split='train', transform=transform_train)
    #     testset = BrainTumorMRIDataset(root='/home/jbb/data/brain_tumor_mri', split='test', transform=transform_test)
