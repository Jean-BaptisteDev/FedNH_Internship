...
class FedAvgClient(Client):
    """
    FedAvgClient: Implements a federated averaging client for local model training 
    and evaluation, uploading its model parameters to the server.
    """
    ...
    def _initialize_model(self):
        """
        Initializes the client model based on the specified configuration.
        Moves model and criterion to the specified device (CPU/GPU).
        """
    ...
    def training(self, round, num_epochs):
        ...
        Performs training on the client's local data for the specified number of epochs.
            
        Parameters:
        - round: The current communication round with the server.
        - num_epochs: Number of epochs to run on local data.

        Stores updated model parameters and training loss/accuracy for each epoch.
        """
    ...
    def upload(self):
        """
        Returns the updated state dictionary (model parameters) to be sent to the server.
        """
        return self.new_state_dict

    def testing(self, round, testloader=None):
        """
        Evaluates the client's model on the test dataset and stores accuracy results.
        
        Parameters:
        - round: Current communication round.
        - testloader: Custom DataLoader for testing, defaults to client's testloader.

        Stores class-wise accuracy and weight statistics for performance evaluation.
        """
        self.model.eval()
...
class FedAvgServer(Server):
    """
    FedAvgServer: Handles federated aggregation of client model updates, global model 
    management, and model evaluation for a federated learning setup.
    """
    ...
    def aggregate(self, client_uploads, round):
        """
        Aggregates client updates to create a new global model state.

        Parameters:
        - client_uploads: List of state dictionaries uploaded by clients.
        - round: Current communication round.

        Updates global model parameters by aggregating client updates.
        """
    ...
    def testing(self, round, active_only=True, **kwargs):
        ...
        Tests the global model and, if specified, local models of active clients.

        Parameters:
        - round: Current communication round.
        - active_only: If True, only tests active clients.

        Evaluates the global model on the test dataset and, if specified, active clients' local models.
    ...
    def collect_stats(self, stage, round, active_only, **kwargs):
        ...
        Collects and averages training or testing statistics across active clients.

        Parameters:
        - stage: Indicates 'train' or 'test' stage.
        - round: Current communication round.
        - active_only: If True, collects stats only from active clients.

        Computes average accuracy and loss metrics and updates global test statistics.
